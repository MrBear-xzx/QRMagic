...
纯前端 Web SPA 二维码美化工具。支持内容编码、码点样式、颜色渐变、Logo 嵌入、边框装饰、模板系统、SVG/PNG 导出、批量生成、历史记录、PWA 离线缓存。仅简体中文。
...
| 模板缩略图 | 内置模板渲染真实 QR 码 SVG 预览图 | ✅ |
| Logo 上传 E2E | 用户手动验证通过，上传流程正常 | ✅ |
...
| 功能 | 说明 |
|------|------|
| 二维码解码 | 下载的 PNG 反向解码验证内容正确性 |

### 🐛 已知问题

1. **微信扫码兼容性**：✅ 已用真机验证通过
2. **边框标签文字过长**：✅ 已修复自动换行 + 高度适配
3. **Logo 上传后不生效**：✅ 已验证通过
4. **大内容编码**：超长文本可能超出 QR 版本 40 限制，暂无错误提示。
5. **切换页签时预览区宽度抖动**：左侧面板 flex-grow 导致内容宽度变化时挤压预览区，待优化。
